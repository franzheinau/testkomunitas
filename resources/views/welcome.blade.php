...
<head>
    ...
    <title>{{ config('app.name', 'Pthree') }}</title>

    <!-- Font -->
    <link rel="preconnect" href="https://fonts.bunny.net">
    <link href="https://fonts.bunny.net/css?family=instrument-sans:400,500,600" rel="stylesheet" />

    <!-- TAILWIND CDN (TIDAK ADA VITE DI SINI) -->
    <script src="https://cdn.tailwindcss.com"></script>

    <style>
        body { font-family: "Instrument Sans", system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; }
    </style>
</head>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16">
            ...
            <div class="flex items-center gap-3">
                @if (Route::has('login'))
                    ...
                    @else
                        <a href="{{ route('login') }}" class="px-4 py-2 text-indigo-600 border border-indigo-200 rounded-md">
                            Log in
                        </a>

                        @if (Route::has('register'))
                            <a href="{{ route('register') }}" class="px-4 py-2 bg-indigo-600 text-white rounded-md ml-2">
                                Register
                            </a>
                        @endif
                    @endauth
        ...
        <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
            <div>
                <h1 class="text-4xl font-extrabold text-gray-900 mb-4">
                    Selamat Datang Di Komunitas Pthree
                </h1>
                <p class="text-lg text-gray-600 mb-6">
                    Komunitas Baca dari SMAN 2 Muara Teweh
                </p>

                <div class="flex flex-wrap gap-3">
                    @if (Route::has('register'))
                        <a href="{{ route('register') }}"
                           class="inline-flex items-center px-5 py-3 bg-indigo-600 text-white rounded-md shadow">
                            Langsung Register aja Yuks
                        </a>
                    @endif
                    <a href="https://www.pthreesmada.my.id"
                       class="inline-flex items-center px-5 py-3 border border-gray-200 text-gray-700 rounded-md">
                        Cek Website Kami Yang lain
                    </a>
                </div>
            ...
            <div>
                <img src="https://ik.imagekit.io/3fgrgeqi6z/pthree.png?updatedAt=1764753926583"
